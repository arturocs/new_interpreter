...
pub fn join(args: &[Variant], _memory: &mut Memory) -> Variant {
    if args.len() != 2 {
    ...
    let result = match &args[0] {
        ...
        Variant::Iterator(i) => i.clone().join(separator),
        _ => return Variant::error("Second argument of join must be a vector or an iterator"),
    };
    Variant::str(result)
}

pub fn map(args: &[Variant], memory: &mut Memory) -> Variant {
    if args.len() != 2 {
        return Variant::error("map function needs two arguments");
    }
    // args[0].clone().map(args[1].clone(), memory).unwrap_or_else(Variant::error)
    todo!()
}
